...
The repository now includes a small Express server (requires Node.js 18+) that proxies requests to a local Llama model served by [Ollama](https://ollama.ai/).
...
2. In a **second terminal**, pull the desired model (only needed the first time) and start the chat server. Leave this terminal running so the front end can reach it:
   ...
   ```sh
    ...
    The server should print something like `* Running on http://0.0.0.0:3001/` when it starts.
   ```
