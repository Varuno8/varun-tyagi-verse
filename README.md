...
```
   ...
   run `npm install` and `pip install -r requirements.txt` again so the servers have everything they need.
...
4. In a **second terminal**, pull the model (only needed the first time) and then run the Python chat API which proxies to your local Llama model. Leave this terminal running:
```sh
ollama pull llama3.2:latest
npm run server
```

   The server should log `* Running on http://0.0.0.0:3001/`.
   Keep this terminal running so the frontend can talk to it.
